...
def compute_total_energy(lattice: np.ndarray, L: int) -> float:
    ...
    padded[0, 1:-1] = lattice[-1, :]  # top row
    padded[-1, 1:-1] = lattice[0, :]  # bottom row
    padded[1:-1, 0] = lattice[:, -1]  # left column
    padded[1:-1, -1] = lattice[:, 0]  # right column

    # Wrap corners
    padded[0, 0] = lattice[-1, -1]
    padded[0, -1] = lattice[-1, 0]
    padded[-1, 0] = lattice[0, -1]
    padded[-1, -1] = lattice[0, 0]
...
def compute_statistics(temp_range: np.ndarray, L_range: np.ndarray, N: int, init_up_rate: float = 0.5, simulation_func=wolff_algorithm) -> dict:
    ...
    for L in L_range:
        ...
        D = []
        for T in temp_range:
            energies, magnetizations = simulation_func(L, rate, N, T)
            energy, hist_E, hist_M, hist_M2, hist_M4 = compute_histogram(energies, magnetizations)
            rate = (np.mean(np.abs(magnetizations)) / L**2 + 1) / 2
            print(L, T)
            D.append([T, energy, hist_E, hist_M, hist_M2, hist_M4])
        
        # Create arrays with dtype=object to allow for varying shapes
...
def binder(data, L, T, Z1):
    _, _, s_m, s_m2, s_m4, _, _, _ = get_avg(data, L, T, Z1)
    return 1.0 - s_m4 / (s_m2 * s_m2 * 3.0)

def dbinder_dbeta(data, L, T, Z1):
    Nsite = L**2
    _, _, s_m, s_m2, s_m4, s_me, s_m2e, s_m4e = get_avg(data, L, T, Z1)
